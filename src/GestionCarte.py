...
def mapTouteFormation(dataframe):
    ...
    for i in dataframe.index:
        folium.CircleMarker(
            location=(float((dataframe["Coordonnées GPS de la formation"][i].split(","))[0]),
                      float((dataframe["Coordonnées GPS de la formation"][i].split(","))[1])),
            radius=2,
...
def mapParFiliereTresAgregee(dataframe, name):
    ...
    for i in lstName.index:
        folium.CircleMarker(
            location=(float((lstName["Coordonnées GPS de la formation"][i].split(","))[0]),
                      float((lstName["Coordonnées GPS de la formation"][i].split(","))[1])),
...
def createAllMap(dataframe):
    lstNameFormation = ["BTS", "Ecole d'Ingénieur", "Licence", "Ecole de Commerce", "CPGE", "BUT", "IFSI", "EFTS",
                        "PASS", ]
