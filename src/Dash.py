...
def init_card(data, date):
    ...
    if date > 2019:
        selectivite = str(
            len(data.loc[(data['Sélectivité'] == "formation sélective")].index) / len(data.index) * 100) + " %"
    else:
        selectivite = "NON DISPONIBLE"

    if date > 2018:
        prct_forma_public = str(len(data.loc[(data[
    ...
    def update_figure(input_value, formation):
        ...
        match input_value:
            ...
            case 3:
                ...
                return fig, True
            case 4:
                fig = create_scatter_chart(data, "Capacité de l’établissement",
                                           'Nombre de formations par établissement')
                return fig, True
